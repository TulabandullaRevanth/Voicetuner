...
const GITHUB_REPO = 'umadevi16177/voice_tuner';
...
export async function getLatestRelease(): Promise<ReleaseInfo> {
  ...
  try {
    ...
    const releaseInfo: ReleaseInfo = {
      ...
      downloadLinks: {
        macArm:
          downloadLinks.macArm || `${baseUrl}/VoiceTuner_${version.replace('v', '')}_aarch64.dmg`,
        macIntel:
          downloadLinks.macIntel || `${baseUrl}/VoiceTuner_${version.replace('v', '')}_x64.dmg`,
        windows:
          downloadLinks.windows || `${baseUrl}/voicetuner_${version.replace('v', '')}_x64_en-US.msi`,
        linux: downloadLinks.linux || `${baseUrl}/voicetuner_x86_64-unknown-linux-gnu.AppImage`,
      },
    };
