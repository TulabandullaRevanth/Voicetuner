...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "voicetuner-server-cuda" / f"voicetuner-server-cuda{exe}",
        DIST_DIR / f"voicetuner-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# VoiceTuner E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="VoiceTuner E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to voicetuner-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="voicetuner-e2e-"))
