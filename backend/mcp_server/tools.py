"""VoiceTuner MCP tool implementations.

Thin wrappers over existing services/routes. Tools are registered with dotted
names (``voicetuner.speak`` etc.) so they look natural in agent logs —
...
def register_tools(mcp: FastMCP) -> None:
    """Attach all VoiceTuner tools to the given FastMCP instance."""

    @mcp.tool(
        name="voicetuner.speak",
        description=(
            "Speak text in a VoiceTuner voice profile. Returns a generation id "
            "the caller can poll at /generate/{id}/status. Audio plays on the "
            "user's speakers and is saved to the Captures / History tab."
        ),
    )
    async def voicetuner_speak(
        text: str,
    ...
    @mcp.tool(
        name="voicetuner.transcribe",
        description=(
            "Transcribe an audio clip to text using VoiceTuner's local Whisper. "
            "Pass exactly one of `audio_base64` (bytes as base64) or "
            "`audio_path` (absolute local file path — loopback callers only)."
        ),
    )
    async def voicetuner_transcribe(
        ...
        # to loopback callers so a VoiceTuner bound on 0.0.0.0 doesn't double
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "Use the returned `name` with voicetuner.speak(profile=...)."
        ),
    )
    async def voicetuner_list_profiles() -> dict[str, Any]:
        db = next(get_db())
        try:
...
async def _transcribe_file(
    ...
    if (
        ...
        raise ValueError(
            ...
            "VoiceTuner → Settings → Models to download it first."
        )
