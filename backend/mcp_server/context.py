...
MCP clients identify themselves via an ``X-VoiceTuner-Client-Id`` HTTP header
(direct-HTTP clients set it in their MCP config; the stdio shim forwards it
from the ``VOICETUNER_CLIENT_ID`` env var). Middleware copies the value into a
...
# host-filesystem access to loopback callers (see voicetuner.transcribe).
...
# Endpoints that consume X-VoiceTuner-Client-Id for its MCP-semantic
...
#
# - /mcp — FastMCP tool calls (voicetuner.speak, voicetuner.transcribe, …)
...
#   false stamps.
# - /speak — REST mirror of voicetuner.speak for non-MCP agents (shell
...
class ClientIdMiddleware(BaseHTTPMiddleware):
    """Copy X-VoiceTuner-Client-Id into a ContextVar and stamp last_seen_at
