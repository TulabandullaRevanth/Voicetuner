...
directly via URL; older stdio-only clients use the ``voicetuner-mcp`` shim
...
def build_mcp_server() -> FastMCP:
    """Create the FastMCP instance with VoiceTuner tools registered."""
    mcp = FastMCP(
        name="voicetuner",
        instructions=(
            "VoiceTuner is a local voice I/O layer. Use `voicetuner.speak` to "
            "play text in a voice profile, `voicetuner.transcribe` for "
...
def compose_lifespan(*lifespans):
    ...
    Used by ``create_app`` to run the existing VoiceTuner startup/shutdown
