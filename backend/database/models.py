...
class VoiceProfile(Base):
    ...
    # Mean GE2E d-vector (JSON list[float]) computed from uploaded samples.
    # Used by the automatic speaker-identification pipeline on every capture.
...
class CaptureSettings(Base):
    ...
    # "VoiceTuner would like to receive keystrokes from any application" dialog
...
class MCPClientBinding(Base):
    ...
    itself via the ``X-VoiceTuner-Client-Id`` HTTP header; direct-HTTP
    clients set it in their MCP config's ``headers`` block, the stdio
    shim forwards it from the ``VOICETUNER_CLIENT_ID`` env var.
    """
    ...
    # When true, voicetuner.speak routes through the profile's personality LLM
...
class Capture(Base):
    ...
    # Speaker identification — set automatically after each STT pass.
