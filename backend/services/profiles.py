...
def _rebuild_speaker_embedding(profile_id: str, db: Session) -> None:
    ...
    try:
        ...
        db.commit()
        if emb is not None:
            logger.info("Rebuilt speaker embedding for profile %s (%d samples)", profile_id, len(audio_paths))
        else:
            logger.debug("No valid audio to embed for profile %s", profile_id)
    except Exception:
        logger.warning("Speaker embedding rebuild failed for profile %s", profile_id, exc_info=True)


def get_profiles_with_embeddings(db: Session) -> list[tuple[str, str, str]]:
    """Return [(profile_id, profile_name, embedding_json)] for all profiles
    that have a stored speaker embedding."""
    rows = db.query(
        DBVoiceProfile.id,
        DBVoiceProfile.name,
        DBVoiceProfile.speaker_embedding,
...
def _profile_to_response(
    ...
    return VoiceProfileResponse(
        ...
        speaker_enrolled=bool(getattr(profile, "speaker_embedding", None)),
...
def _get_preset_voice_ids(engine: str) -> set[str]:
    ...
    if engine == "sarvam":
        from ..adapters.sarvam import SARVAM_VOICES

        return {voice_id for voice_id, _name, _gender, _lang, _desc in SARVAM_VOICES}
...
async def add_profile_sample(
    ...
    # Rebuild speaker embedding in a thread — best-effort, never blocks response
    import asyncio
    asyncio.get_event_loop().run_in_executor(
        None, _rebuild_speaker_embedding, profile_id, db
    )
...
async def delete_profile_sample(
    ...
    # Rebuild speaker embedding — sample set changed
    import asyncio
    asyncio.get_event_loop().run_in_executor(
        None, _rebuild_speaker_embedding, profile_id, db
    )
