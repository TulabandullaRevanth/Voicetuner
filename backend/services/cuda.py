...
Downloads two archives from GitHub Releases:
  1. Server core (voicetuner-server-cuda.tar.gz) — the exe + non-NVIDIA deps,
...
GITHUB_RELEASES_URL = "https://github.com/umadevi16177/voice_tuner/releases/download"
...
def get_cuda_exe_name() -> str:
    ...
    if sys.platform == "win32":
        return "voicetuner-server-cuda.exe"
    return "voicetuner-server-cuda"
...
def is_cuda_active() -> bool:
    ...
    """
    return os.environ.get("VOICETUNER_BACKEND_VARIANT") == "cuda"
...
async def _download_cuda_binary_locked(version: Optional[str] = None):
    ...
    server_archive = "voicetuner-server-cuda.tar.gz"
...
def get_cuda_binary_version() -> Optional[str]:
    ...
    try:
        ...
        # Output format: "voicetuner-server 0.3.0"
        for line in result.stdout.strip().splitlines():
            if "voicetuner-server" in line:
                return line.split()[-1]
    except Exception as e:
