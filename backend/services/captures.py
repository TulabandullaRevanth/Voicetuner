...
from .profiles import get_profiles_with_embeddings
...
def _to_response(row: DBCapture) -> CaptureResponse:
    ...
    return CaptureResponse(
        ...
        identified_profile_id=getattr(row, "identified_profile_id", None),
        identified_profile_name=getattr(row, "identified_profile_name", None),
        speaker_confidence=getattr(row, "speaker_confidence", None),
...
async def create_capture(
    *,
    ...
    try:
        ...
        whisper = get_stt_backend_for_language(language)
        # Cloud STT (Sarvam/Groq) has no Whisper model size to resolve.
        resolved_stt = (
            (stt_model or whisper.model_size)
            if hasattr(whisper, "load_model_async")
            else None
        )
        transcript = await whisper.transcribe(str(audio_path), language, resolved_stt)

        # Speaker identification — match audio against all stored voice profiles
        identified_profile_id = None
        identified_profile_name = None
        speaker_confidence = None
        try:
            from ..adapters.speaker_id import identify_speaker
            candidates = get_profiles_with_embeddings(db)
            if candidates:
                result = await asyncio.to_thread(identify_speaker, str(audio_path), candidates)
                if result:
                    identified_profile_id, identified_profile_name, speaker_confidence = result
                    logger.info(
                        "Capture %s identified speaker: %s (%.2f)",
                        capture_id, identified_profile_name, speaker_confidence,
                    )
        except Exception:
            logger.debug("Speaker identification failed for capture %s", capture_id, exc_info=True)
        ...
        row = DBCapture(
            ...
            identified_profile_id=identified_profile_id,
            identified_profile_name=identified_profile_name,
