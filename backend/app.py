...
def create_app() -> FastAPI:
    ...
    @asynccontextmanager
    async def voicetuner_lifespan(app: FastAPI):
    ...
    # compose_lifespan enters factories in order (voicetuner startup →
    ...
    lifespan = compose_lifespan(voicetuner_lifespan, mcp_app.router.lifespan_context)

    application = FastAPI(
        title="voicetuner API",
...
def _configure_cors(application: FastAPI) -> None:
    ...
    env_origins = os.environ.get("VOICETUNER_CORS_ORIGINS", "")
...
async def _run_startup(application: FastAPI) -> None:
    ...
    logger.info("VoiceTuner v%s starting up", __version__)
    logger.info(
        "Python %s on %s %s (%s)",
        sys.version.split()[0],
        platform.system(),
...
async def _run_shutdown() -> None:
    ...
    logger.info("VoiceTuner server shutting down...")
    try:
