...
import { Edit2, Fingerprint, Mic, X } from 'lucide-react';
      ...
      <div className={cn('flex-1 overflow-y-auto', isPlayerVisible && BOTTOM_SAFE_AREA_PADDING)}>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-0">
            ...
            {/* Speaker Recognition — 3-state quality signal */}
            {profile.voice_type === 'cloned' && (() => {
              const sampleCount = profile.sample_count ?? 0;
              const srState = !profile.speaker_enrolled
                ? 'notEnrolled'
                : sampleCount < 3
                  ? 'limited'
                  : 'active';
              return (
                <div className="px-5 pb-4 pt-2 border-t border-border/50 mx-5">
                  <div className="flex items-center gap-2 mb-1.5 mt-3">
                    <Fingerprint
                      className={cn(
                        'h-4 w-4 shrink-0',
                        srState === 'active' && 'text-emerald-400',
                        srState === 'limited' && 'text-amber-400',
                        srState === 'notEnrolled' && 'text-muted-foreground',
                      )}
                    />
                    <span className="text-sm font-medium">{t('speakerRecognition.title')}</span>
                    {srState === 'active' && (
                      <Badge className="h-5 px-1.5 text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium">
                        {t('speakerRecognition.active')}
                      </Badge>
                    )}
                    {srState === 'limited' && (
                      <Badge className="h-5 px-1.5 text-[10px] bg-amber-500/10 text-amber-400 border border-amber-500/20 font-medium">
                        {t('speakerRecognition.limited')}
                      </Badge>
                    )}
                    {srState === 'notEnrolled' && (
                      <Badge variant="outline" className="h-5 px-1.5 text-[10px] text-muted-foreground font-medium">
                        {t('speakerRecognition.notEnrolled')}
                      </Badge>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    {t(`speakerRecognition.${srState}Hint`)}
                  </p>
                </div>
              );
            })()}
