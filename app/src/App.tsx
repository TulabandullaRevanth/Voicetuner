...
/**
 * Validate that a health response has the expected VoiceTuner-specific shape.
 * Prevents misidentifying an unrelated service on the same port.
 */
function isVoiceTunerHealthResponse(health: HealthResponse): boolean {
  return (
...
function MainApp() {
  ...
  useEffect(() => {
    if (!platform.metadata.isTauri) {
      ...
      if (currentServerUrl !== serverUrl && isLoopbackVoiceTunerServerUrl(currentServerUrl)) {
    ...
    if (!import.meta.env?.PROD) {
      ...
      window.__voicetunerServerStartedByApp = false;
      return;
    }
    ...
    platform.lifecycle
      ...
      .then((serverUrl) => {
        ...
        window.__voicetunerServerStartedByApp = true;
      })
      .catch((error) => {
        ...
        // until it responds with a valid VoiceTuner payload, then transition to
        // the main UI.
        console.log('Falling back to health-check polling...');
        const pollInterval = setInterval(async () => {
          try {
            const health = await apiClient.getHealth();
            if (!isVoiceTunerHealthResponse(health)) {
              console.log('Health response is not from a VoiceTuner server, keep polling...');
              return;
            }
            console.log('External VoiceTuner server detected via health check');
        ...
        setTimeout(() => {
          ...
          serverStartingRef.current = false;
          setStartupError(
            'Could not connect to a VoiceTuner server within 2 minutes. ' +
