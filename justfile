# VoiceTuner development commands
...
build-server: _ensure-venv
    ...
    Copy-Item "backend/dist/voicetuner-server.exe" "{{ tauri_dir }}/src-tauri/binaries/voicetuner-server-$triple.exe" -Force; \
    Write-Host "Copied sidecar: voicetuner-server-$triple.exe"
...
build-server-cuda: _ensure-venv
    ...
    $dest = "$env:APPDATA/sh.voicetuner.app/backends/cuda"; \
    if (Test-Path $dest) { Remove-Item -Recurse -Force $dest }; \
    New-Item -ItemType Directory -Path $dest -Force | Out-Null; \
    Copy-Item "backend/dist/voicetuner-server-cuda/*" $dest -Recurse -Force; \
...
[unix]
db-reset:
    rm -f {{ backend_dir }}/data/voicetuner.db
    just db-init

[windows]
db-reset:
    if (Test-Path "{{ backend_dir }}/data/voicetuner.db") { Remove-Item -Force "{{ backend_dir }}/data/voicetuner.db" }
    just db-init
