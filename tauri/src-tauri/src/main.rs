...
/// Find a voicetuner-server process listening on a given port (Windows only).
///
/// Uses PowerShell `Get-NetTCPConnection` to look up the PID owning the port,
/// then verifies via `tasklist` that it's a voicetuner process. The caller is
...
#[cfg(windows)]
fn find_voicetuner_pid_on_port(port: u16) -> Option<u32> {
...
/// Check if a VoiceTuner server is responding on the given port.
///
/// Sends an HTTP GET to `/health` and returns `true` only if the response
/// is valid JSON matching the VoiceTuner `HealthResponse` schema — specifically
        ...
        .build()
    {
        Ok(client) => match client.get(&url).send() {
            Ok(resp) => {
                ...
                // Parse as JSON and validate VoiceTuner-specific fields
...
async fn start_server(
    ...
    // Check if a voicetuner server is already running on our port (from previous session with keep_running=true,
            ...
            .output()
        {
            ...
            for line in output_str.lines().skip(1) {
                ...
                if parts.len() >= 2 {
                    let command = parts[0];
                    let pid_str = parts[1];
                    if command.contains("voicetuner") {
                        if let Ok(pid) = pid_str.parse::<u32>() {
                            println!("Found existing voicetuner-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                            // Store the PID so we can kill it on exit if needed
                            *state.server_pid.lock().unwrap() = Some(pid);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                    } else {
                        // Process name doesn't contain "voicetuner" — could be an external
                        // Python/uvicorn/Docker server. Verify via HTTP health check.
                        println!("Port {} in use by '{}' (PID: {}), checking if it's a VoiceTuner server...", SERVER_PORT, command, pid_str);
                        if check_health(SERVER_PORT) {
                            println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                        println!("Health check failed — port is occupied by a non-VoiceTuner process");
                        return Err(format!(
                            "Port {} is already in use by another application ({}). \
                             Close it or change the VoiceTuner server port.",
                            SERVER_PORT, command
                        ));
                    }
                }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            // Port is in use — check if it's a voicetuner process by name first
            if let Some(pid) = find_voicetuner_pid_on_port(SERVER_PORT) {
                println!("Found existing voicetuner-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                *state.server_pid.lock().unwrap() = Some(pid);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            ...
            println!("Port {} in use by unknown process, checking if it's a VoiceTuner server...", SERVER_PORT);
            if check_health(SERVER_PORT) {
                println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            return Err(format!(
                "Port {} is already in use by another application. \
                 Close the other application or change the VoiceTuner port.",
                SERVER_PORT
            ));
        }
    }

    // Kill any orphaned voicetuner-server from previous session on legacy port 8000
                ...
                if parts.len() >= 2 {
                    ...
                    if command.contains("voicetuner") {
                        if let Ok(pid) = pid_str.parse::<i32>() {
                            println!("Found orphaned voicetuner-server on legacy port {} (PID: {}, CMD: {}), killing it...", LEGACY_PORT, pid, command);
                        ...
                        println!("Legacy port {} is in use by non-voicetuner process: {} (PID: {}), not killing", LEGACY_PORT, command, pid_str);
                    }
                }
            }
        }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            if let Some(pid) = find_voicetuner_pid_on_port(LEGACY_PORT) {
                println!("Found orphaned voicetuner-server on legacy port {} (PID: {}), killing it...", LEGACY_PORT, pid);
    ...
    println!("=================================================================");
    println!("Starting voicetuner-server sidecar");
    ...
    let cuda_binary = {
        ...
        let cuda_name = if cfg!(windows) {
            "voicetuner-server-cuda.exe"
        } else {
            "voicetuner-server-cuda"
        };
        ...
        if exe_path.exists() {
            ...
            {
                Ok(output) => {
                    // Output format: "voicetuner-server X.Y.Z\n"
    ...
    let sidecar_result = app.shell().sidecar("voicetuner-server");
    ...
    let spawn_result = if let Some(ref cuda_path) = cuda_binary {
        ...
        if let Some(ref dir) = effective_models_dir {
            cmd = cmd.env("VOICETUNER_MODELS_DIR", dir);
        }
        cmd.spawn()
    } else {
        ...
        if let Some(ref dir) = effective_models_dir {
            sidecar = sidecar.env("VOICETUNER_MODELS_DIR", dir);
        }
        ...
        match tokio::time::timeout(tokio::time::Duration::from_millis(100), rx.recv()).await {
            ...
            Ok(None) => {
                ...
                #[cfg(not(debug_assertions))]
                {
                    ...
                    eprintln!("Check Console.app logs for more details (search for 'voicetuner')");
...
/// Identifier of the VoiceTuner app itself — used to short-circuit auto-paste
/// when the user fires a chord while focus was inside one of our own
/// windows. Paste into VoiceTuner-internal targets is step 6 territory and
...
#[cfg(target_os = "macos")]
const VOICETUNER_BUNDLE_ID: &str = "sh.voicetuner.app";
#[cfg(target_os = "windows")]
const VOICETUNER_BUNDLE_ID: &str = "voicetuner.exe";
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
const VOICETUNER_BUNDLE_ID: &str = "voicetuner";
...
///
/// On macOS this is the call that triggers the "VoiceTuner would like to receive
...
/// - `focus.bundle_id` is VoiceTuner itself — step 6 will inject directly
...
async fn paste_final_text(
    ...
    if focus.bundle_id.as_deref() == Some(VOICETUNER_BUNDLE_ID) {
        return Ok(false);
    }
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission required for auto-paste. Open System Settings → Privacy & Security → Accessibility and enable VoiceTuner."
                .into(),
        );
    }
    ...
    if safe_to_restore {
        ...
        eprintln!(
            "[voicetuner] clipboard mutated during paste window — skipping restore to preserve newer content"
        );
    }
...
async fn debug_focus_roundtrip(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable VoiceTuner."
                .into(),
        );
    }
...
async fn debug_paste_text(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable VoiceTuner, then try again."
                .into(),
        );
    }
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            #[cfg(desktop)]
            {
                ...
                // steals focus to the VoiceTuner app when the user clicks
                ...
                // Agent-initiated speech (voicetuner.speak over MCP or POST /speak)
