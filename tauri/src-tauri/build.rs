...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        // voicetuner.icon is in tauri/assets/voicetuner.icon (one level up from src-tauri)
        let icon_source = format!("{}/../assets/voicetuner.icon", project_root);
        ...
        if std::path::Path::new(&icon_source).exists() {
            ...
            // Generate voicetuner.icns from the source PNG via sips + iconutil
            let icns_path = format!("{}/voicetuner.icns", gen_dir);
            if !std::path::Path::new(&icns_path).exists() {
                let source_png = format!("{}/Assets/VoiceTuner.png", icon_source);
                if std::path::Path::new(&source_png).exists() {
                    let iconset_dir = format!("{}/voicetuner.iconset", gen_dir);
                    ...
                    match iconutil_output {
                        Ok(out) if out.status.success() => {
                            println!("Generated voicetuner.icns");
                        }
                        Ok(out) => {
    ...
    {
        let required = ["Assets.car", "voicetuner.icns", "partial.plist"];
