---
title: "Building"
description: "How VoiceTuner is built for production"
---

## Overview

VoiceTuner uses a two-stage build process:
...
cp dist/voicetuner-server ../tauri/src-tauri/binaries/voicetuner-server-${PLATFORM}
```
...
```json
{
  "tauri": {
    "bundle": {
      "externalBin": ["binaries/voicetuner-server"]
    }
  }
}
```

Tauri looks for `voicetuner-server-${PLATFORM}` in `src-tauri/binaries/` and bundles it.
...
3. Package with `scripts/package_cuda.py` into two archives:
   - `voicetuner-server-cuda.tar.gz` — server core (~945 MB)
...
<AccordionGroup>
  ...
  <Accordion title="Tauri can't find sidecar">
    Ensure binary exists at `tauri/src-tauri/binaries/voicetuner-server-${PLATFORM}` before running Tauri build.
  </Accordion>
</AccordionGroup>
