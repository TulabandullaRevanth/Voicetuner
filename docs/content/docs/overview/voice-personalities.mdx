...
The model runs through the same backend split VoiceTuner already uses for TTS
...
<Steps>
  ...
  <Step title="Speak in character — toggle persona rewrite">
    ...
    generate — VoiceTuner runs the text through the personality LLM first,
...
- **Agents that speak in a voice you own.** Combine the persona toggle with
  ...
  personality. The agent calls `voicetuner.speak({ text, profile, personality:
  true })` and VoiceTuner rewrites the text in character before speaking.
...
`voicetuner.speak` tool uses when you pass `personality: true`. Scripts and
