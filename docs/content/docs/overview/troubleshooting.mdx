...
description: "Common issues and solutions for VoiceTuner"
---

This guide covers common issues you might encounter when using or developing VoiceTuner, along with solutions.
...
xattr -cr /Applications/VoiceTuner.app
```
...
<AccordionGroup>
  ...
  <Accordion title="Permission Issues">
    ...
    ```bash
    # macOS/Linux
    chmod +x ~/Library/Application\ Support/sh.voicetuner.app/backend/voicetuner-server
    ```
  </Accordion>
  ...
  <Accordion title="Check Logs">
    ...
    **macOS:**
    ```bash
    tail -f ~/Library/Application\ Support/sh.voicetuner.app/logs/server.log
    ```

    **Windows:**
    ```bash
    type %APPDATA%\sh.voicetuner.app\logs\server.log
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Install flash-attn on Windows (community wheels)">
    ...
    Alternatively, run VoiceTuner's backend inside WSL2 and use the standard Linux wheels.
  </Accordion>
</AccordionGroup>
...
**Solutions:**
- Close all VoiceTuner instances
- Delete the lock file:
  ```bash
  # macOS
  rm ~/Library/Application\ Support/sh.voicetuner.app/data/voicetuner.db-shm
  rm ~/Library/Application\ Support/sh.voicetuner.app/data/voicetuner.db-wal
  ```
...
```bash
# macOS
rm ~/Library/Application\ Support/sh.voicetuner.app/data/voicetuner.db

# Windows
del %APPDATA%\sh.voicetuner.app\data\voicetuner.db
```
...
<AccordionGroup>
  ...
  <Accordion title="Check GPU Selection">
    If you have multiple GPUs, ensure VoiceTuner is using the right one.
...
1. **Check GitHub Issues:** [github.com/umadevi16177/voice_tuner/issues](https://github.com/umadevi16177/voice_tuner/issues)
2. **Open a New Issue:** Provide:
   - Operating system and version
   - VoiceTuner version
   - Steps to reproduce
   - Error messages or logs
3. **Join Discord:** [discord.gg/voicetuner](https://discord.gg/voicetuner) (coming soon)
