...
description: "Run VoiceTuner as a headless server with a web UI using Docker"
---

## Overview

VoiceTuner can run as a Docker container with a full web UI -- no desktop app required. This is ideal for headless servers, shared GPU machines, or self-hosted deployments.
...
Open [http://localhost:17493](http://localhost:17493) in your browser. The full VoiceTuner UI is served directly from the backend.
...
| `VOICETUNER_MODELS_DIR` | (HuggingFace cache) | Custom path for model storage |
| `VOICETUNER_CORS_ORIGINS` | (local origins) | Additional CORS origins, comma-separated |
...
| `voicetuner-data` | `/app/data` | Profiles, database, cache |
| `huggingface-cache` | `/home/voicetuner/.cache/huggingface` | Downloaded models (persists across rebuilds) |
...
- **Non-root user** -- the server runs as `voicetuner`, not `root`
...
For production deployments, put VoiceTuner behind nginx or Caddy with TLS and authentication:

```nginx
server {
    listen 443 ssl;
    server_name voicetuner.example.com;

    ssl_certificate /etc/ssl/certs/voicetuner.pem;
    ssl_certificate_key /etc/ssl/private/voicetuner.key;

    auth_basic "VoiceTuner";
...
If you see `{"message": "voicetuner API", ...}` instead of the web UI, the frontend build may have failed during the Docker build. Check the build logs:
...
docker run -p 17493:17493 ghcr.io/umadevi16177/voice_tuner:latest
```
