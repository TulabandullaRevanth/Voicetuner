...
VoiceTuner ships a built-in **Model Context Protocol** server so local AI
agents can call your VoiceTuner install directly: speak text in a voice
profile, transcribe audio, and list captures or profiles. The server runs
inside the same process as the rest of VoiceTuner and is mounted at `/mcp`
over Streamable HTTP.

Agent asks to speak → VoiceTuner plays audio on your speakers → an on-screen
...
VoiceTuner → Settings → MCP. The value is just an identifier for the
per-client voice binding — not a secret, not a credential.

### Clients that only speak stdio

A stdio shim binary `voicetuner-mcp` is bundled with the desktop app. Point
...
<Tabs items={["macOS", "Windows", "Linux"]}>
  ...
  ```json
  {
    "mcpServers": {
      "voicetuner": {
        "command": "C:\\Program Files\\VoiceTuner\\voicetuner-mcp.exe",
        "env": { "VOICETUNER_CLIENT_ID": "claude-desktop" }
      }
    }
  }
  ...
  ```json
  {
    "mcpServers": {
      "voicetuner": {
        "command": "/opt/voicetuner/voicetuner-mcp",
        "env": { "VOICETUNER_CLIENT_ID": "claude-desktop" }
      }
    }
  }
...
The shim waits up to 30 seconds for the VoiceTuner backend to come up, then
proxies JSON-RPC from stdio over Streamable HTTP. VoiceTuner must be running
...
| Tool | Use |
|---|---|
| `voicetuner.speak` | Speak text in a voice profile. Returns a `generation_id` to poll. |
| `voicetuner.transcribe` | Whisper transcription of base64 audio or an absolute local path. |
| `voicetuner.list_captures` | Recent captures with transcripts, paginated. |
...
Every call to `voicetuner.speak` (and `POST /speak`) resolves the voice profile
in this order:
...
<Steps>
  ...
  <Step title="Per-client binding">
    Looked up by the `X-VoiceTuner-Client-Id` header. Managed in
    **VoiceTuner → Settings → MCP**. Lets you pin Claude Code to Morgan,
...
VoiceTuner → Settings → MCP shows one row per `client_id` VoiceTuner has heard
...
| `default_personality` | When true, `voicetuner.speak` routes through the profile's personality LLM (rewrite) by default. |
...
silent background TTS is a trust hazard, so VoiceTuner always shows what's
...
Start with `voicetuner.list_profiles` to confirm wiring, then
`voicetuner.speak` for end-to-end — you should hear audio and see the
generation land in the Captures tab.

<Callout type="info">
  If an agent can't reach the server, the first thing to check is that
  VoiceTuner is running — the backend only listens while the desktop app is
...
- **Localhost only.** The server binds to `127.0.0.1`. If you ever point
  VoiceTuner at a non-loopback interface (e.g. remote-mode over a trusted
...
- **No auth today.** Any process that can connect to your loopback can
  call MCP. That's the same trust boundary as the rest of VoiceTuner's REST
...
- **Voice cloning consent applies.** See [Voice Cloning](/overview/voice-cloning#limitations)
  — an agent being able to call `voicetuner.speak` in someone's voice
...
- **Lifespan:** mounting FastMCP requires the `lifespan=` kwarg on
  ...
  with FastMCP's Streamable HTTP session manager. The VoiceTuner app.py
