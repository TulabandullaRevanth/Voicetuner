...
a chord, talk, release — VoiceTuner transcribes what you said with Whisper,
...
<Callout type="info">
  ...
  per-profile personality modes. It's the "input" half of VoiceTuner's voice I/O
...
<Steps>
  ...
  <Step title="Text lands in your app">
    If auto-paste is enabled and VoiceTuner has Accessibility permission, the
...
VoiceTuner ships two chord behaviors out of the box:
...
- **Left vs right modifier badges.** When you hold keys into the chord
  picker, VoiceTuner records whether each modifier is the left or right variant.
...
Once transcription finishes, VoiceTuner can synthesize a native paste into
...
- Whisper loop hallucinations — VoiceTuner strips repeated tokens (six or
...
- **UAC / UIPI caveat.** Synthetic paste into an *elevated* window from a
  non-elevated VoiceTuner is blocked by Windows itself. Run VoiceTuner elevated
...
A few cases where VoiceTuner deliberately does *not* synthesize a paste:

- **Focus was inside VoiceTuner** when the chord started. The transcript goes
  to the Captures tab so a dictation-into-VoiceTuner round-trip doesn't
