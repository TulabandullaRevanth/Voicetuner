...
**Issue:** [#10 OpenAI API compatibility](https://github.com/umadevi16177/voice_tuner/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from VoiceTuner, allowing any tool, library, or application that speaks the OpenAI Audio API to use VoiceTuner as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph voicetuner [VoiceTuner Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with VoiceTuner
- **LLM frameworks**: LangChain, AutoGen, etc. can use VoiceTuner for TTS
...
- `voice` parameter maps to VoiceTuner profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a VoiceTuner profile.
