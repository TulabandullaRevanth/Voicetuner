...
Docker support makes VoiceTuner easier to deploy, especially for:
...
```bash
# CPU-only version
docker run -p 8000:8000 -v voicetuner-data:/app/data \
  ghcr.io/umadevi16177/voice_tuner:latest

# NVIDIA GPU version
docker run --gpus all -p 8000:8000 -v voicetuner-data:/app/data \
  ghcr.io/umadevi16177/voice_tuner:latest-cuda

# AMD GPU version (experimental)
docker run --device=/dev/kfd --device=/dev/dri -p 8000:8000 \
  -v voicetuner-data:/app/data \
  ghcr.io/umadevi16177/voice_tuner:latest-rocm
```
...
services:
  voicetuner:
    image: ghcr.io/umadevi16177/voice_tuner:latest-cuda
    ports:
      - "8000:8000"
    volumes:
...
Build and run:
```bash
docker build -t voicetuner .
docker run -p 8000:8000 -v $(pwd)/data:/app/data voicetuner
```
...
Build:
```bash
docker build -f Dockerfile.optimized -t voicetuner:slim .
```
...
docker run --device=/dev/kfd --device=/dev/dri \
  ...
  -p 8000:8000 -v voicetuner-data:/app/data \
  voicetuner:rocm
```
...
```bash
docker run -v voicetuner-data:/app/data \           # Profiles, generations, history
           -v huggingface-cache:/root/.cache/huggingface \  # Downloaded models
           -p 8000:8000 voicetuner
```
...
Configure VoiceTuner via environment variables:
...
doctl compute droplet create voicetuner \
...
sh get-docker.sh
docker run --gpus all -d -p 80:8000 voicetuner:cuda
```
...
```bash
# Build and push
docker build -t gcr.io/your-project/voicetuner .
docker push gcr.io/your-project/voicetuner

# Deploy to Cloud Run
gcloud run deploy voicetuner \
  --image gcr.io/your-project/voicetuner \
  --platform managed \
  --region us-central1 \
  --memory 4Gi \
  --cpu 2 \
...
[build]
  image = "ghcr.io/umadevi16177/voice_tuner:latest"
...
```bash
# Fix permissions
docker run --user $(id -u):$(id -g) -v $(pwd)/data:/app/data voicetuner
```
...
```bash
docker run -e GPU_MEMORY_FRACTION=0.5 voicetuner
```

Or use CPU-only:
```bash
docker run -e DEVICE=cpu voicetuner
```
...
```bash
docker run -v huggingface-cache:/root/.cache/huggingface voicetuner
```

Or use host cache:
```bash
docker run -v ~/.cache/huggingface:/root/.cache/huggingface voicetuner
```
...
Change host port:
```bash
docker run -p 8080:8000 voicetuner  # Use port 8080 instead
```
...
```dockerfile
RUN useradd -m -u 1000 voicetuner
USER voicetuner
```
...
docker run -e GPU_MEMORY_FRACTION=0.8 voicetuner

# Allow GPU memory growth (prevents OOM)
docker run -e TF_FORCE_GPU_ALLOW_GROWTH=true voicetuner
```
...
View container logs:
```bash
docker logs -f voicetuner

# Or with compose
docker compose logs -f voicetuner
```
...
4. Report issues: [GitHub Issues](https://github.com/umadevi16177/voice_tuner/issues)
