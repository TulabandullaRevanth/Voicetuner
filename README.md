<p align="center">
  <img src=".github/assets/icon-dark.webp" alt="VoiceTuner" width="120" height="120" />
</p>

<h1 align="center">VoiceTuner</h1>

<p align="center">
  <strong>The open-source trilingual AI voice studio.</strong><br/>
  Clone voices, generate speech, and dictate — in English, Hindi &amp; Telugu.<br/>
  Local-first for English, cloud-assisted (Sarvam) for Indic languages.
</p>

<p align="center">
  <a href="https://github.com/umadevi16177/voice_tuner/releases">
    <img src="https://img.shields.io/github/downloads/umadevi16177/voice_tuner/total?style=flat&color=blue" alt="Downloads" />
  </a>
  <a href="https://github.com/umadevi16177/voice_tuner/releases/latest">
    <img src="https://img.shields.io/github/v/release/umadevi16177/voice_tuner?style=flat" alt="Release" />
  </a>
  <a href="https://github.com/umadevi16177/voice_tuner/stargazers">
    <img src="https://img.shields.io/github/stars/umadevi16177/voice_tuner?style=flat" alt="Stars" />
  </a>
  <a href="https://deepwiki.com/umadevi16177/voice_tuner">
    <img src="https://img.shields.io/static/v1?label=Ask&message=DeepWiki&color=5B6EF7" alt="Ask DeepWiki" />
  </a>
</p>

<p align="center">
  <a href="https://voicetuner.app">voicetuner.app</a> •
  <a href="https://docs.voicetuner.app">Docs</a> •
...
<p align="center">
  <a href="https://voicetuner.app">
    <img src="landing/public/assets/app-screenshot-1.webp" alt="VoiceTuner App Screenshot" width="800" />
  </a>
</p>

<p align="center">
  <em>Click the image above to watch the demo video on <a href="https://voicetuner.app">voicetuner.app</a></em>
</p>

<br/>

<p align="center">
  <img src="landing/public/assets/app-screenshot-2.webp" alt="VoiceTuner Screenshot 2" width="800" />
</p>

<p align="center">
  <img src="landing/public/assets/app-screenshot-3.webp" alt="VoiceTuner Screenshot 3" width="800" />
</p>
...
VoiceTuner is a **trilingual AI voice studio** for **English, Hindi, and Telugu** — clone voices, generate speech, dictate into any text field with a global hotkey, and give any MCP-aware AI agent a voice of your choosing. It runs local-first for English, with a cloud tier for the Indic languages local engines can't serve.

**Languages & providers:**

- **English** — fully local (Qwen3-TTS, Chatterbox, Kokoro, etc.); cloud optional
- **Hindi & Telugu TTS/STT** — [Sarvam AI](https://sarvam.ai) (Bulbul TTS + Saarika STT), purpose-built for Indian languages. No local engine can synthesize Telugu.
- **STT** — local Whisper (English) or [Groq](https://groq.com) hosted Whisper-large-v3 / Sarvam Saarika (all three languages)
- **Voice cloning** — local Chatterbox for English/Hindi (free, on-device); Telugu cloning requires [ElevenLabs](https://elevenlabs.io) Instant Voice Cloning (paid plan)

See [`SETUP.md`](SETUP.md) for the free-tier quickstart (only a Sarvam key required).

- **Local-first for English** — models and voice data stay on your machine; Indic languages route to Sarvam
- **Multiple TTS engines** — Qwen3-TTS, Chatterbox, Kokoro, and more for English; Sarvam Bulbul preset voices for en/hi/te
- **Voice cloning and preset voices** — on-device cloning (en/hi) plus Sarvam preset voices for Telugu/Hindi/English
...
- **Agent voice output** — one tool call (`voicetuner.speak`) and any MCP-aware agent (Claude Code, Cursor, Cline) speaks to you in a voice you've cloned
...
| Platform              | Download                                               |
| --------------------- | ------------------------------------------------------ |
| macOS (Apple Silicon) | [Download DMG](https://voicetuner.app/download/mac-arm)   |
| macOS (Intel)         | [Download DMG](https://voicetuner.app/download/mac-intel) |
| Windows               | [Download MSI](https://voicetuner.app/download/windows)   |
| Docker                | `docker compose up`                                    |

> **[View all binaries →](https://github.com/umadevi16177/voice_tuner/releases/latest)**

> **Linux** — Pre-built binaries are not yet available. See [voicetuner.app/linux-install](https://voicetuner.app/linux-install) for build-from-source instructions.
...
Multiple TTS engines with different strengths, switchable per-generation. **VoiceTuner restricts output to English, Hindi, and Telugu** — the "Languages" column below reflects each engine's upstream capability; Hindi/Telugu are served primarily by Sarvam (added as a cloud engine), since the local engines below cannot synthesize Telugu:
...
The other half of the voice I/O loop. Hold a hotkey anywhere on your system, speak, release — on macOS the transcript pastes straight into the focused text field. Or hit the mic on any VoiceTuner text input and dictate directly into the app.
...
- **In-app mic button** on every VoiceTuner text field — generation form, profile descriptions, story titles, anywhere you'd type
...
VoiceTuner runs OpenAI Whisper for transcription — the same model that backs dictation, the Captures tab, and the `/transcribe` API. Running on MLX (Apple Silicon) or PyTorch (CUDA / ROCm / DirectML / CPU) depending on your platform.
...
- **Local capture storage** — original audio and transcript stay in your VoiceTuner data directory, with a folder shortcut in Settings
...
- **HTTP + stdio transports** — install as a URL in Claude Code / Cursor / Windsurf / VS Code MCP, or point stdio-only clients at the bundled `voicetuner-mcp` binary
...
Agents can reach the same rewrite path over MCP by passing `personality: true` to `voicetuner.speak`, turning the tool into a text-in → personality-LLM → TTS pipeline. The same LLM backs dictation's refinement step — one LLM in the app, one model cache, one GPU-memory footprint.
...
- Custom models directory via `VOICETUNER_MODELS_DIR`
...
VoiceTuner exposes a REST API for integrating voice I/O into your own apps and agents.
...
curl -X POST http://127.0.0.1:17493/speak \
  -H "Content-Type: application/json" \
  -H "X-VoiceTuner-Client-Id: my-script" \
...
VoiceTuner ships a built-in **Model Context Protocol** server so any MCP-aware agent (Claude Code, Cursor, Windsurf, Cline, VS Code MCP extensions) can speak, transcribe, and browse captures and profiles.
...
```json
{
  "mcpServers": {
    "voicetuner": {
      "url": "http://127.0.0.1:17493/mcp",
      "headers": { "X-VoiceTuner-Client-Id": "cursor" }
    }
  }
}
```

**Stdio fallback** for clients that don't speak HTTP MCP — point at the bundled `voicetuner-mcp` binary inside the app:
...
```json
{
  "mcpServers": {
    "voicetuner": {
      ...
      "env": { "VOICETUNER_CLIENT_ID": "claude-desktop" }
    }
  }
}
```

Four tools ship: `voicetuner.speak`, `voicetuner.transcribe`, `voicetuner.list_captures`, `voicetuner.list_profiles`. Per-client voice bindings are managed in **VoiceTuner → Settings → MCP**. See the [full MCP guide](docs/content/docs/overview/mcp-server.mdx) for tool signatures, resolution precedence, the speaking-pill contract, and security notes.
...
| **Mobile companion**               | Control VoiceTuner from your phone                                         |
